...
const NewProducts = () => {
  ...
  useEffect(() => {
    const fetchNewProducts = async () => {
        try {
            const res = await axios.get('http://localhost:4000/api/product/new-products');
            setNewCollections(res.data.newProducts);
        } catch (error) {
    ...
    <Container className="py-5">
      ...
      <div className="position-relative mb-4">
        ...
        <div className="d-flex justify-content-between align-items-center position-relative">
          ...
          >
            <span style={{color: '#ff6b35'}}>GAME</span> MỚI
          </h1>
      ...
      <Row className="g-4 justify-content-center">
        {newCollections.slice(-4).map((product, index) => (
          <Col 
            key={index}
